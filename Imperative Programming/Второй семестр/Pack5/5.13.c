...
int main() {
    ...
    int *queries = (int *)malloc(sizeof(int) * k * 2);
    ...
    for (int i = 0; i < k; ++i) {
        Bellman_Ford(&graph, queries[2 * i] - 1, distances, parent);
        if (distances[queries[2 * i + 1] - 1] == 1000000000) {
            printf("DOOMED\n");
        }
        else {
            printf("quarantine ");
            vert = queries[2 * i + 1] - 1;
            while (vert != queries[2 * i] - 1) {
                path[len] = parent[graph.edges[parent[vert]].to] + 1;
                ++len;
                vert = graph.edges[parent[vert]].from;
            }
            printf("%d %d", distances[queries[2 * i + 1] - 1], len);
            while (len > 0) {
                printf(" %d", path[--len]);
            }
            printf("\n");
